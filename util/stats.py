    ...
    def print_stats(self, oil):
        ...
        if self.config.combat['enabled']:
            ...
            if self.combat_done >= 1:
                Logger.log_success("Gold, oil (battle): {}, {}".format(
                ...
                if self.oil_battle_accumulated != 0:
                    ...
                    Logger.log_success("Average gold/oil (battle): {number1:.{digit1}f} / {number2:.{digit2}f}".format(
                        number1=self.gold_battle_accumulated/self.combat_done, digit1=2, 
                        number2=self.oil_battle_accumulated/self.combat_done, digit2=2))
                # g/o from retirement
                Logger.log_success("Average gold/oil (retire): {number1:.{digit1}f} / {number2:.{digit2}f}".format(
                ...
                if self.oil_battle_accumulated + self.oil_retire_accumulated != 0:
                    Logger.log_success("Average gold/oil (total):  {number1:.{digit1}f} / {number2:.{digit2}f} = {number3:.{digit3}f}".format(
                    ...
                    Logger.log_success("Average gold/oil (total):  {number1:.{digit1}f} / {number2:.{digit2}f}".format(
                        number1=(self.gold_battle_accumulated + self.gold_retire_accumulated)/self.combat_done, digit1=2,
                        number2=(self.oil_battle_accumulated + self.oil_retire_accumulated)/self.combat_done, digit2=2))
