...
class RetirementModule(object):
    ...
    def handle_retirement(self):
        ...
        Logger.log_info('Beginning of handle_retirement. Start_time:{}'.format(start_time))
        ...
        while True:
            ...
            Utils.update_screen()
            time = datetime.now()
            Logger.log_info('Beginning of while loop. Time:{}'.format(time))

            if timedelta(minutes=5) < time - start_time:
                Logger.log_info('Time limit triggered')
            ...
            if Utils.find_with_cropped("retirement/alert_bonus"):
                Logger.log_info('retirement/alert_bonus triggered')
                Utils.touch_randomly(self.region['confirm_selected_ships_button'])
                Utils.script_sleep(self.sleep_time_long)
                continue
            if Utils.find_with_cropped("menu/item_found"):
                Logger.log_info('menu/item_found triggered. item_found={}'.format(items_found))
                Utils.touch_randomly(self.region['tap_to_continue'])
                Utils.script_sleep(self.sleep_time_long)
                items_found += 1
                if items_found > 1:
                    Logger.log_info('Leaving handle_retirement')
                    return
                continue
            if Utils.find_with_cropped("menu/alert_info"):
                Logger.log_info('menu/item_alert_info triggered')
            ...
            if Utils.find_with_cropped("retirement/button_disassemble"):
                Logger.log_info('retirement/button_disassemble triggered')
